...
const PastEvents: FC<EventProps> = ({
  activeMenu,
  createEvent,
  collection,
  authorized,
  ...
  const loadFunc = (): void => {
    const eventParams: AllEventsPayload = {
      id: collectionId,
      collection,
      //   page: page + 1,
      //   pageSize: 20,
      authToken,
      startDate: moment().subtract(100, 'days').format('YYYY-MM-DD HH:mm'),
      endDate: moment().format('YYYY-MM-DD HH:mm'),
    };
    setPage((p) => p + 1);
    switch (collection) {
      case 'performers':
        dispatch(eventActions.fetchEventByPerformerRange(eventParams));
        break;
      case 'sabhas':
        dispatch(eventActions.fetchEventBySabhaRange(eventParams));
        break;
      case 'venues':
        dispatch(eventActions.fetchEventByVenueRange(eventParams));
        break;
      default:
    }
  };
            ...
            <TitleRow>
              ...
              <RightActions>
                <Button onClick={createEvent}>
                  ...
                  <span className="txt">View Event</span>
                </Button>
              </RightActions>
            </TitleRow>
          </Card.Header>
